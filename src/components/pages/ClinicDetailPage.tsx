...
export default function ClinicDetailPage({ id }: ClinicDetailPageProps) {
  const { data, loading, locale } = useContent();
  const isRtl = locale === 'ar';

  const clinic = data.clinics?.find((c: any) => c._id === id);
    ...
    return (
      <>
        ...
        <main className="flex-grow pt-32 pb-24 bg-slate-950 flex flex-col items-center justify-center text-cyan-400 min-h-screen">
          ...
          <span className="text-sm font-bold">{isRtl ? 'جاري تحميل ملف المنشأة...' : 'Loading profile...'}</span>
        </main>
        <Footer />
      </>
    );
    ...
    return (
      <>
        ...
        <main className="flex-grow pt-32 pb-24 bg-slate-950 flex flex-col items-center justify-center text-slate-400 min-h-screen text-center px-6 select-none">
          <span className="material-symbols-outlined text-6xl text-slate-650 mb-4">domain_disabled</span>
          <h1 className="text-2xl font-bold text-white mb-2">{isRtl ? 'لم يتم العثور على المنشأة' : 'Clinic/Center Not Found'}</h1>
          <p className="text-xs max-w-sm mb-8 leading-relaxed">
            {isRtl ? 'عذراً، الملف الشخصي لهذه المنشأة غير موجود أو تم إزالته.' : 'Sorry, this clinic profile does not exist or has been removed.'}
          </p>
          <Link href={isRtl ? '/clinics' : '/en/clinics'} className="bg-cyan-500 hover:bg-cyan-400 text-slate-950 px-6 py-2.5 rounded-xl font-bold text-xs transition-transform duration-300">
            {isRtl ? 'العودة لصفحة المراكز والعيادات' : 'Back to Clinics'}
          </Link>
        </main>
        <Footer />
      </>
  ...
  const specialty = isRtl ? clinic.specialty_ar : clinic.specialty_en;
  const desc = isRtl ? clinic.desc_ar : clinic.desc_en;
  const certsRaw = isRtl ? clinic.certificates_ar : clinic.certificates_en;

  // Parse accreditations split by newlines
      ...
      <main className={`flex-grow pt-32 pb-24 overflow-x-hidden bg-slate-950 ${isRtl ? 'text-right' : 'text-left'} text-slate-350 selection:bg-cyan-500 selection:text-slate-900 animate-fade-in relative`}>
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-12 relative z-10">
          ...
          >
            ...
            <span>{isRtl ? 'العودة لقائمة المراكز' : 'Back to Clinics List'}</span>
          </Link>

          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-start">

            <div className="lg:col-span-5 flex flex-col items-center">
              <div className="w-full max-w-sm aspect-video bg-transparent rounded-[2.5rem] relative overflow-hidden group select-none shadow-2xl mb-8">
                {/* Glowing glow decoration */}
                <div className="absolute -inset-1 bg-gradient-to-r from-cyan-400 to-indigo-500 rounded-[2.5rem] opacity-20 blur group-hover:opacity-30 transition duration-1000 -z-10"></div>
                
                {clinic.image_url ? (
                   <img 
                     src={clinic.image_url} 
                     alt={name} 
                     className="w-full h-full object-contain hover:scale-[1.03] transition-transform duration-700 ease-out animate-float-slow"
                   />
                ) : (
                  <div className="w-full h-full bg-slate-950/80 rounded-[2rem] flex items-center justify-center">
                    <span className="material-symbols-outlined text-slate-700 text-6xl">
                      local_hospital
                    </span>
                  </div>
                )}
              ...
              <div className="w-full max-w-sm space-y-3.5 select-none">
                <a
                  href={`https://wa.me/${(data.content['phone'] || '966560875412').replace(/[^0-9]/g, '')}?text=${encodeURIComponent(isRtl ? `مرحباً، أود حجز موعد استفسار للعيادة: ${name}` : `Hello, I would like to book an inquiry for: ${name}`)}`}
                  target="_blank"
                ...
                >
                  ...
                  <span>{isRtl ? 'تواصل فوري عبر الواتساب' : 'Inquire via WhatsApp'}</span>
                </a>
                <Link
                ...
                >
                  ...
                  <span>{isRtl ? 'طلب استشارة تسويقية للمركز' : 'Request Center Consultation'}</span>
                </Link>
              </div>
            </div>

            {/* Right Column: Profile copy details */}
            <div className="lg:col-span-7 space-y-8">
              ...
              <div>
                ...
                <p className="text-sm text-slate-400 mt-2 font-medium">
                  {isRtl ? 'شريك طبي معتمد لدى ديجيتال هيلث' : 'Certified Healthcare Partner Center'}
                </p>
              </div>
              ...
              <div className="glass-card p-8 rounded-[2rem] border border-white/5 space-y-4">
                <div className="flex items-center gap-2 text-white font-bold text-sm mb-2">
                  <span className="material-symbols-outlined text-cyan-400 text-lg">medical_information</span>
                  <span>{isRtl ? 'نبذة عن المنشأة والخدمات' : 'About the Clinic/Center'}</span>
                </div>
                <p className="text-xs md:text-sm text-slate-300 leading-[1.8] whitespace-pre-wrap">
                  {desc || (isRtl ? 'لا يوجد نبذة تفصيلية مسجلة حالياً.' : 'No description registered currently.')}
                </p>
              </div>

              {/* Accreditations Credentials List */}
              <div className="glass-card p-8 rounded-[2rem] border border-white/5 space-y-6">
                <div className="flex items-center gap-2 text-white font-bold text-sm">
                  ...
                  <span>{isRtl ? 'الشهادات والاعتمادات الرسمية' : 'Accreditations & Certificates'}</span>
                </div>
                ...
                {certificates.length > 0 ? (
                  <ul className="space-y-4 pr-1">
                    {certificates.map((cert: string, idx: number) => (
                      <li key={idx} className="flex gap-3.5 items-start text-xs md:text-sm text-slate-350">
                        <span className="material-symbols-outlined text-emerald-400 shrink-0 text-lg relative top-0.5">verified</span>
                  ...
                  <p className="text-xs text-slate-500 italic">
                    {isRtl ? 'لم يتم إدراج اعتمادات رسمية بعد.' : 'No official accreditations listed yet.'}
                  </p>
                )}
              </div>
